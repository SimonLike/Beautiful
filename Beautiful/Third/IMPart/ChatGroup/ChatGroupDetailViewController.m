...
//    [self.navigationItem setLeftBarButtonItem:backItem];
    ...
    UIButton *backButton = [Utils createButtonWith:CustomButtonType_Back text:nil];
    //[backButton setImage:[UIImage imageNamed:@"vps_actionbar_black_ic3.png"] forState:UIControlStateNormal];
    [backButton addTarget:self action:@selector(back) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *backItem = [[UIBarButtonItem alloc] initWithCustomView:backButton];
    [self.navigationItem setLeftBarButtonItem:backItem];
    self.tableView = [[UITableView alloc] initWithFrame:self.view.bounds style:UITableViewStyleGrouped];
    self.tableView.backgroundColor = [Utils colorWithHexString:@"f0f2f5"];
...
{
    NSString* str = @"";
    for (NSString* idStr in _chatGroup.occupants) {
        if([idStr isEqualToString:@"admin"])
            groupNum += 0;
        else
            groupNum += 1;
        if([str isEqualToString:@""])
        {
            if ([idStr isEqualToString:@"admin"])
                ...
                str = [str stringByAppendingString:idStr];
        }
        else
        {
            if ([idStr isEqualToString:@"admin"])
                ...
                str = [str stringByAppendingString:[NSString stringWithFormat:@",%@",idStr]];
        }
    }
//    [[NetWork shareInstance] groupInfo:[Utils readUnarchiveHistoryGoodsVOsAry].token UserIds:str GroupId:_chatGroup.groupId callBack:^(MKNetworkOperation *completedOperation) {
//        NSError *error;
...
//            {
//                imInfoAry = [[NSMutableArray alloc] init];
//                if([[dic[@"data"] objectForKey:@"groupInfo"] objectForKey:@"createTime"])
//                    groupTime = [[dic[@"data"] objectForKey:@"groupInfo"] objectForKey:@"createTime"];
//                NSMutableArray* listAry = [dic[@"data"] objectForKey:@"userInfos"];
//                for (int i = 0; i<listAry.count; i++) {
//                    NSDictionary* infoDic = listAry[i];
//                    IMUserInfoVO* iVO = [[IMUserInfoVO alloc] init];
//                    iVO.name = infoDic[@"name"];
//                    iVO.iconUrl = infoDic[@"iconUrl"];
//                    iVO.userId = [infoDic[@"userId"] stringValue];
//                    [imInfoAry addObject:iVO];
//                }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        cell.textLabel.text = [NSString stringWithFormat:@"全部会议室成员（%i）",(int)groupNum];
    ...
    {
        cell.textLabel.text = [NSString stringWithFormat:@"创建时间：%@",groupTime];//[NSString stringWithFormat:@"全室成员（%i）",(int)[_chatGroup.occupants count]];
        cell.accessoryType = UITableViewCellAccessoryNone;
...
- (void)reloadDataSource
{
    ...
    NSMutableArray* memAry = [[NSMutableArray alloc] init];
    for (NSString* idStr in self.chatGroup.occupants) {
        if(![idStr isEqualToString:@"admin"])
            [memAry addObject:idStr];
    }
    
    [self.dataSource addObjectsFromArray:memAry];
    ...
    for (i = 0; i < row; i++) {
        for (j = 0; j < kColOfRow; j++) {
            ...
            if (index < [self.dataSource count]) {
                ...
                IMUserInfoVO* iVO;
                for (IMUserInfoVO* imVO in imInfoAry) {
                    if([username isEqualToString:@"admin"])
                        continue;
                    if([username isEqualToString:imVO.userId])
                    {
                        iVO = imVO;
                        break;
                    }
                ...
                //contactView.image = [UIImage imageNamed:@"defaulthead.png"];
                contactView.remark = iVO.name;
                [contactView.imageView sd_setImageWithURL:[NSURL URLWithString:iVO.iconUrl]  placeholderImage:[UIImage imageNamed:@"defaulthead.png"]];
        ...
        for (ContactView *contactView in self.scrollView.subviews)
        {
            ...
            if (CGRectContainsPoint(contactView.bounds, locaton))
            {
                if ([contactView isKindOfClass:[ContactView class]]) {
                    ...
                    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:NSLocalizedString(@"delete", @"deleting member..."), NSLocalizedString(@"friend.block", @"add to black list"), nil];
