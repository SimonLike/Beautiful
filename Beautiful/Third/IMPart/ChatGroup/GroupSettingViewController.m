...
- (void)isIgnoreGroup:(BOOL)isIgnore
{
    ...
    [[EaseMob sharedInstance].chatManager asyncIgnoreGroupPushNotification:_group.groupId isIgnore:isIgnore completion:^(NSArray *ignoreGroupsList, EMError *error) {
        //[weakSelf hideHud];
        if (!error) {
            [[XQTipInfoView getInstanceWithNib]appear:NSLocalizedString(@"group.setting.success", @"set success")];
        }
        else{
            [[XQTipInfoView getInstanceWithNib]appear:NSLocalizedString(@"group.setting.fail", @"set failure")];
        }
    } onQueue:nil];
}
