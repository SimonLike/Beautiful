...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        if (!error) {
            ...
            [[XQTipInfoView getInstanceWithNib]appear:[NSString stringWithFormat:@"删除失败：%@", error.description]];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex != actionSheet.cancelButtonIndex && _currentLongPressIndex) {
        ...
        [[EaseMob sharedInstance].chatManager asyncBlockBuddy:buddy.username relationship:eRelationshipBoth withCompletion:^(NSString *username, EMError *error){
            ...
            {
                [[XQTipInfoView getInstanceWithNib]appear:error.description];
            }
        } onQueue:nil];
    }
...
- (void)cellImageViewLongPressAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:NSLocalizedString(@"friend.block", @"join the blacklist") otherButtonTitles:nil, nil];
