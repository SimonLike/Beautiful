...
- (void)setModel:(MessageModel *)model
{
    ...
    if(self.model.isSender)
    {
        _textLabel.textColor = [Utils colorWithHexString:@"666666"];//修改文字消息颜色
    }
...
//                                                      }
//                                            context:nil].size;
        UILabel* content = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 200, 20)];
        content.numberOfLines = 999;
        content.font = [UIFont systemFontOfSize:14];
        NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:object.content];
        NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
        
        [paragraphStyle setLineSpacing:0];//调整行间距
        //[paragraphStyle setFirstLineHeadIndent:40];
        [attributedString addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:NSMakeRange(0, [object.content length])];
        content.attributedText= attributedString;
        //self.facilityLabel.font = [UIFont systemFontOfSize:14];
    ...
    return 2 * BUBBLE_VIEW_PADDING + size.height+ 25;
}
