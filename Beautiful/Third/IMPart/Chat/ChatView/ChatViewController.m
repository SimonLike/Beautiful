...
@interface ChatViewController ()<UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SRRefreshDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate, LocationViewDelegate, IDeviceManagerDelegate,PriceViewDelegate>
{
...
- (void)setupBarButtonItem
{
    ...
    if (_isChatGroup) {
        ...
        [detailButton setImage:[UIImage imageNamed:@"info.png"] forState:UIControlStateNormal];
        [detailButton setImage:[UIImage imageNamed:@"info_press.png"] forState:UIControlStateNormal];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if (model.isSender) {
                model.headImageURL =  [NSURL URLWithString:[Utils readUnarchiveHistoryGoodsVOsAry].iconUrl];
                model.username = [Utils readUnarchiveHistoryGoodsVOsAry].username;
            }
            else
            {
                if(model.message.ext[@"iconUrl"]&&![model.message.ext[@"iconUrl"] isEqualToString:@""])
                    model.headImageURL = [NSURL URLWithString:model.message.ext[@"iconUrl"]];
                if(model.message.ext[@"nickName"]&&![model.message.ext[@"nickName"] isEqualToString:@""])
                    model.username = model.message.ext[@"nickName"];
            }
...
-(void)chatLocationCellBubblePressed:(MessageModel *)model
{
    ...
    [self showHint:[model.message.ext objectForKey:@"C"]];
...
-(void)chatImageCellBubblePressed:(MessageModel *)model
{
    ...
    //[self showHint:[model.message.ext objectForKey:@"type"]];
    __weak ChatViewController *weakSelf = self;
    id <IChatManager> chatManager = [[EaseMob sharedInstance] chatManager];
...
{
    [self sendPriceImageMessage:orgImage dic:ext];
}
// by HQL
...
- (void)moreViewPhotoAction:(DXChatBarMoreView *)moreView
{
    ...
    [Utils setNavBarBgUI:self.imagePicker.navigationBar];
...
-(void)sendLocationLatitude:(double)latitude longitude:(double)longitude andAddress:(NSString *)address
{
    ...
    EMMessage *locationMessage = [ChatSendHelper sendLocationLatitude:latitude longitude:longitude address:address toUsername:_conversation.chatter isChatGroup:_isChatGroup requireEncryption:NO ext:@{@"C":@"靠"}];
    //locationMessage.ext =  ;
...
//    }
    ...
    [nameDic setObject:[Utils readUnarchiveHistoryGoodsVOsAry].username forKey:@"nickName"];
    [nameDic setObject:[Utils readUnarchiveHistoryGoodsVOsAry].iconUrl forKey:@"iconUrl"];
...
-(void)sendPriceImageMessage:(UIImage *)imageMessage dic:(NSDictionary*)ext
{
