...
+(void)startSendImageName:(NSString *)name image:(UIImage *)image parameters:(NSDictionary *)parameters block:(AsynchronousBlock)block
{
    ...
    //NSLog(@"URL--> %@",[NSString stringWithFormat:@"%@%@/%@",BASED_URL,module,function]);
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://%@/%@",API_HOST,@"base!upload.action"]];
    ...
    NSString *fileName = [NSString stringWithFormat:@"%@-%lld-%@.png",/*[Utils readUnarchiveHistoryGoodsVOsAry].token*/@"asdfasf",num,code];//命名，必须唯一
    ////添加分界线，换行
    [body appendFormat:@"%@\r\n",MPboundary];
