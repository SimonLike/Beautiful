...
@interface ProductByVC ()<OrderDeliveryCellDelegate,OrderPayTypeCellDelegate>

@end

@implementation ProductByVC
...
- (IBAction)commit:(id)sender
{
    ...
    ProductByVC * __weak weakSelf = self;
    [[NetWork shareInstance] OrderPay:[Utils readUnarchiveHistoryGoodsVOsAry].token AddrId:_currentVO.addrId Products:ary DeliveryType:_currentVO.deliveryType CashCouponId:_currentVO.cashCouponID PayType:self.channel orderNo:_currentVO.orderNo CallBack:^(BOOL isSucc, NSDictionary *info) {
       if(isSucc)
       {
           
           
           
//           long long amount = [info[@"data"][@"amount"] intValue];
//           if (amount == 0)
//           {
//               return;
//           }
...
//           [Pingpp createPayment:info[@"data"] viewController:weakSelf appURLScheme:@"wxdc1e388c3822c80b" withCompletion:^(NSString *result, PingppError *error) {
//               NSLog(@"completion block: %@", result);
//               if (error == nil) {
//                   NSLog(@"PingppError is nil");
//               } else {
