...
@interface AppointServiceVC ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate, CategoryChooseViewDelegate>
{
    int page;
    int pageSize;
...
@implementation AppointServiceVC
...
- (void)getOrderData:(int)Page
    {
        ...
        [[NetWork shareInstance] PinList:zID Cid:cID Page:page PageSize:pageSize CallBack:^(BOOL isSucc, NSDictionary *info) {
            if(isSucc)
            {
                if(page == 1)
                {
                ...
                NSArray* ary = [TeamVO objectArrayWithKeyValuesArray:info[@"data"][@"items"]];
                [proTable.mj_header endRefreshing];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    static NSString *cellIdentifier = @"PingCell";
    PingCell *cell = (PingCell*)[tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (cell == nil) {
        cell = [PingCell getInstanceWithNib];
    }
    else{
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
    {
        ...
        NSInteger row = indexPath.row;
        TeamVO* tVo = dataAry[row];
        [[NetWork shareInstance] PinDetial:tVo.teamProductId CallBack:^(BOOL isSucc, NSDictionary *info) {
            if(isSucc)
            {
                TeamVO* vo = [TeamVO objectWithKeyValues:info[@"data"]];
                vo.pic = tVo.pic;
                AppointServiceDetialVC* vc = [[AppointServiceDetialVC alloc] initWithNibName:@"AppointServiceDetialVC" bundle:nil];
                vc.currentVO = vo;
                //vc.currentpVO = teamProductId;
